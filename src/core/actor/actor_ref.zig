...
pub const ActorRef = struct {
    ...
    pub const VTable = struct {
        tell: *const fn (self: *ActorRef, message: anytype, sender: ?*ActorRef) anyerror!void,
        ask: *const fn (self: *ActorRef, message: anytype, timeout_ms: u64) anyerror!*Message,
    ...
    pub fn tell(self: *Self, message: anytype, sender: ?*ActorRef) !void {
        try self.vtable.tell(self, message, sender);
    }
    
    pub fn ask(self: *Self, message: anytype, timeout_ms: u64) !*Message {
...
pub const LocalActorRef = struct {
    ...
    fn ask(actor_ref: *ActorRef, message: anytype, timeout_ms: u64) !*Message {
        ...
        // 发送消息
        const msg = try Message.create(actor_ref.allocator, message);
        try self.actor.send(msg, temp_actor);
    ...
    fn compareTo(actor_ref: *ActorRef, other: *ActorRef) bool {
        ...
        return std.mem.eql(u8, self.remote_address, other_remote.remote_address) and
               std.mem.eql(u8, self.actor_ref.path.toString(self.actor_ref.allocator) catch "", 
                          other.path.toString(other.allocator) catch "");
    }
};
...
pub const MessageSerializer = struct {
    ...
    pub const VTable = struct {
        serialize: *const fn (self: *MessageSerializer, message: anytype, allocator: Allocator) anyerror![]u8,
    ...
    pub fn serialize(self: *MessageSerializer, message: anytype, allocator: Allocator) ![]u8 {
        return self.vtable.serialize(self, message, allocator);
    }
