...
const ObjectPoolConfig = @import("../../utils/memory.zig").ObjectPoolConfig;
...
pub const HighPerfMailbox = struct {
    ...
    pub fn init(allocator: Allocator, config: MailboxConfig) !Self {
        ...
        if (config.use_memory_pool) {
            const pool_config = ObjectPoolConfig.default(config.pool_size orelse 1000);
            message_pool = try ObjectPool(Message).init(allocator, pool_config);
        }
    ...
    pub fn receiveMessage(self: *Self) ?Message {
        ...
        // 更新head指针，使用seq_cst确保强排序
        const next_head = (current_head + 1) & self.mask;
        self.head.store(next_head, .seq_cst);
