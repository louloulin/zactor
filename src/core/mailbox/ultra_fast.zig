...
pub const UltraFastMailbox = struct {
    ...
    const Ring = struct {
        buffer: []Message,
        head: std.atomic.Value(u32) align(64), // 缓存行对齐
    ...
    ring_selector: std.atomic.Value(u32) align(64),

    // 预分配的消息池
    message_pool: []Message,
    pool_head: std.atomic.Value(u32) align(64),
    pool_capacity: u32,
    ...
    pub fn init(allocator: Allocator, config: MailboxConfig) !Self {
        ...
        return Self{
            .rings = rings,
            .ring_selector = std.atomic.Value(u32).init(0),
            .message_pool = message_pool,
            .pool_head = std.atomic.Value(u32).init(0),
    ...
    inline fn selectRing(self: *Self) *Ring {
        ...
        const timestamp = @as(u32, @truncate(@as(u64, @intCast(std.time.nanoTimestamp()))));
    ...
    pub fn sendMessage(self: *Self, message: Message) !void {
        ...
        while (attempts < NUM_RINGS) : (attempts += 1) {
            ...
            if (next_tail != current_head) {
                ...
                // 使用最强的内存屏障确保顺序 - 通过更强的内存排序替代fence

                // 原子更新tail
                if (ring.tail.cmpxchgWeak(current_tail, next_tail, .seq_cst, .seq_cst) == null) {
                    if (self.stats) |*stats| {
                        stats.incrementSent();
    ...
    pub fn receiveMessage(self: *Self) ?Message {
        ...
        for (&self.rings) |*ring| {
            ...
            if (current_head != current_tail) {
                ...
                // 使用内存屏障确保读取完成 - 通过更强的内存排序替代fence

                // 原子更新head
                const next_head = (current_head + 1) & ring.mask;
                if (ring.head.cmpxchgWeak(current_head, next_head, .seq_cst, .seq_cst) == null) {
                    if (self.stats) |*stats| {
    ...
    pub fn allocateMessage(self: *Self) ?*Message {
        ...
        if (self.pool_head.cmpxchgWeak(current_head, next_head, .seq_cst, .seq_cst) == null) {
