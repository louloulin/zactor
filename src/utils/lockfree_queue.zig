...
pub fn LockFreeQueue(comptime T: type) type {
    return struct {
        ...
        // 调试信息
        pub fn debugInfo(self: *const Self) struct { head: u32, tail: u32, size: u32, capacity: u32 } {
            return .{
    ...
    return struct {
        ...
        pub fn push(self: *Self, item: T) bool {
            while (true) {
                ...
                // 尝试原子性地更新head
                if (self.head.cmpxchgWeak(head, next_head, .seq_cst, .seq_cst) == null) {
                    self.buffer[head] = item;
                    return true;
                }
        ...
        pub fn pop(self: *Self) ?T {
            while (true) {
                ...
                // 尝试原子性地更新tail
                if (self.tail.cmpxchgWeak(tail, next_tail, .seq_cst, .seq_cst) == null) {
