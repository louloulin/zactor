...
const CounterActor = struct {
        ...
        switch (message.message_type) {
            .user => {
                ...
                if (std.mem.eql(u8, message.data.user.payload, "\"increment\"")) {
                    ...
                    std.log.info("Counter '{}' current count: {}", .{ self.name, self.count });
                } else {
                    std.log.warn("Counter '{}' received unknown message: {s}", .{ self.name, message.data.user.payload });
                }
            },
            .system => {
                switch (message.data.system) {
                    .ping => {
                        std.log.info("Counter '{}' received ping, sending pong", .{self.name});
                        try context.sendSystem(context.getSelf(), .pong);
                    },
                    .pong => {
                        std.log.info("Counter '{}' received pong", .{self.name});
                    },
                    else => {},
                }
            },
            .control => {
                std.log.info("Counter '{}' received control message", .{self.name});
            },
        }
    }
    ...
    pub fn preStart(self: *Self, context: *zactor.Actor.ActorContext) !void {
        _ = context;
        std.log.info("Counter '{}' starting with initial count: {}", .{ self.name, self.count });
    }
    
    pub fn postStop(self: *Self, context: *zactor.Actor.ActorContext) !void {
        _ = context;
        std.log.info("Counter '{}' stopping with final count: {}", .{ self.name, self.count });
    }
    
    pub fn preRestart(self: *Self, context: *zactor.Actor.ActorContext, reason: anyerror) !void {
        _ = context;
        std.log.info("Counter '{}' restarting due to: {}, count was: {}", .{ self.name, reason, self.count });
    }
    
    pub fn postRestart(self: *Self, context: *zactor.Actor.ActorContext) !void {
        _ = context;
        std.log.info("Counter '{}' restarted, count reset to: {}", .{ self.name, self.count });
    }
};
