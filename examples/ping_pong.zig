...
const PingActor = struct {
        ...
        switch (message.message_type) {
            .user => {
                ...
                if (self.tryHandlePongMessage(message, context)) {
                    ...
                    std.log.warn("PingActor '{s}' received unknown message", .{self.name});
                }
            },
            .system => {
                switch (message.data.system) {
                    .start => {
                        std.log.info("PingActor '{s}' received start signal", .{self.name});
    ...
    fn tryHandlePongMessage(self: *Self, message: zactor.Message, context: *zactor.ActorContext) bool {
        ...
        if (std.mem.startsWith(u8, message.data.user.payload, "\"pong:")) {
            ...
            if (count < self.max_pings) {
                ...
                std.log.info("PingActor '{s}' finished ping-pong game after {} rounds", .{ self.name, count });
            }
            return true;
        }
        return false;
    ...
    fn tryHandlePartnerMessage(self: *Self, message: zactor.Message, context: *zactor.ActorContext) bool {
        if (std.mem.startsWith(u8, message.data.user.payload, "\"partner:")) {
            ...
            if (context.system.findActor(partner_id)) |partner| {
                ...
                std.log.info("PingActor '{s}' set partner to actor {}", .{ self.name, partner_id });
    ...
    fn sendPing(self: *Self, partner: zactor.ActorRef, context: *zactor.ActorContext) !void {
        self.ping_count += 1;
    ...
    pub fn preStart(self: *Self, context: *zactor.ActorContext) !void {
        _ = context;
        std.log.info("PingActor '{s}' starting", .{self.name});
    }

    pub fn postStop(self: *Self, context: *zactor.ActorContext) !void {
        _ = context;
        std.log.info("PingActor '{s}' stopping after {} pings", .{ self.name, self.ping_count });
    }

    pub fn preRestart(self: *Self, context: *zactor.ActorContext, reason: anyerror) !void {
        _ = context;
        std.log.info("PingActor '{s}' restarting due to: {}", .{ self.name, reason });
    }

    pub fn postRestart(self: *Self, context: *zactor.ActorContext) !void {
        _ = context;
        std.log.info("PingActor '{s}' restarted", .{self.name});
    }
};
...
const PongActor = struct {
        ...
        switch (message.message_type) {
            .user => {
                if (self.tryHandlePingMessage(message, context)) {
                    ...
                    std.log.warn("PongActor '{s}' received unknown message", .{self.name});
                }
            },
            .system => {},
            .control => {},
    ...
    fn tryHandlePingMessage(self: *Self, message: zactor.Message, context: *zactor.ActorContext) bool {
        if (std.mem.startsWith(u8, message.data.user.payload, "\"ping:")) {
            ...
            std.log.info("PongActor '{s}' received ping #{}", .{ self.name, count });
    ...
    fn sendPong(self: *Self, sender: zactor.ActorRef, count: u32, context: *zactor.ActorContext) !void {
        self.pong_count += 1;
        const pong_msg = try std.fmt.allocPrint(context.allocator, "pong:{}", .{count});
        defer context.allocator.free(pong_msg);

        std.log.info("PongActor '{s}' sending pong #{}", .{ self.name, count });
        try sender.send([]const u8, pong_msg, context.allocator);
    }

    pub fn preStart(self: *Self, context: *zactor.ActorContext) !void {
        _ = context;
        std.log.info("PongActor '{s}' starting", .{self.name});
    }

    pub fn postStop(self: *Self, context: *zactor.ActorContext) !void {
        _ = context;
        std.log.info("PongActor '{s}' stopping after {} pongs", .{ self.name, self.pong_count });
    }

    pub fn preRestart(self: *Self, context: *zactor.ActorContext, reason: anyerror) !void {
        _ = context;
        std.log.info("PongActor '{s}' restarting due to: {}", .{ self.name, reason });
    }

    pub fn postRestart(self: *Self, context: *zactor.ActorContext) !void {
        _ = context;
        std.log.info("PongActor '{s}' restarted", .{self.name});
    }
};
